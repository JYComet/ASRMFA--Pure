...
│   ├── ctc_prealign.py            # NVASR CTC 强制对齐 → MFA 锚点 (step 4)
│   ├── adjust_ctc_boundaries.py   # 能量分析边界修正 (step 6)
│   ├── postprocess_textgrids.py   # 后处理: 5 层构建 + 质检 + BGM (step 8)
│   ├── prepare_corpus.py          # 语料准备: wav+txt → 拼音
│   ├── convert_dict_to_ipa.py     # 词典: 拼音 → IPA
...
│   └── visualize.py              # Web 可视化 (波形 + 频谱 + TextGrid)
├── dict/
...
| 4 | `normalize` | (内联) | cn2an 阿拉伯数字→中文数字 |
...
NVV (如 LAUGHTER, BREATHING) 后的省略号 `…` 如果包含可听能量 (≥30% 帧 RMS > 噪声底×3)，则合并到 NVV，仅留 60ms 作为标点标记。
...
| `word_in_silence` | 能量 < 噪声底 × 2.0 | 词标在静音区域 |
...
postprocess:
  ...
  detect_bgm: true
  bgm_noise_floor_ratio: 2.0    # 静音能量 > 噪声底 × N → 可疑
  bgm_min_sil_dur: 0.3          # 最小静音段检查时长
  bgm_speech_ratio: 1.0         # 静音能量 > 语音 × N → 可疑
...
```bash
python scripts/view_in_praat.py --dir output       # 浏览 output/ 中的 TextGrid
python scripts/view_in_praat.py --dir aligned      # 浏览 aligned/
```

### Web 可视化 (`visualize.py`)

基于浏览器的波形+频谱+TextGrid 可视化:

```bash
python scripts/visualize.py                              # 启动服务器
python scripts/visualize.py --praat-dir output/          # 批量用 Praat 打开
```
